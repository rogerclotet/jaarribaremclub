...
final class Version20190224232430 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__link AS SELECT id, title, description, url FROM link');
        $this->addSql('DROP TABLE link');
        $this->addSql('CREATE TABLE link (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, title VARCHAR(255) NOT NULL COLLATE BINARY, description VARCHAR(2047) DEFAULT NULL COLLATE BINARY, url VARCHAR(1023) NOT NULL COLLATE BINARY)');
        $this->addSql('INSERT INTO link (id, title, description, url) SELECT id, title, description, url FROM __temp__link');
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('CREATE INDEX IDX_63540594B89032C ON files (post_id)');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_635405993CB796C ON files (file_id)');
        $this->addSql('ALTER TABLE link ADD COLUMN image VARCHAR(1023) DEFAULT NULL COLLATE BINARY');
        $this->addSql('CREATE TEMPORARY TABLE __temp__next_caminada AS SELECT id, number, datetime, place FROM next_caminada');
        $this->addSql('DROP TABLE next_caminada');
